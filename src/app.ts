...
app.post('/approve', async (c) => {
  ...
  if (action === 'login_approve') {
    ...
    if (false) {
      return c.html(
        layout(await renderAuthorizationRejectedContent('/'), 'MCP Remote Auth Demo - Authorization Status'),
      )
    }
  }
  ...
  return c.html(
    layout(await renderAuthorizationApprovedContent(redirectTo), 'MCP Remote Auth Demo - Authorization Status'),
  )
})
